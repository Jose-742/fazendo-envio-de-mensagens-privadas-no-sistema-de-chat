...
public class BlockingChatServerApp {
  ...
  //metodo para checar se a mensagem tem o @, se tiver manda somente para um destinatario
    private boolean checkMsg(final ClientSocket remetente, String msg) {
    	String destinatario = "";
    	String mensagem = "";
    	if(msg.charAt(0) == '@') {
    		String[] arrayMsg = msg.split(" ");
    		String[] arrayLogin = arrayMsg[0].split("@");
    		destinatario = arrayLogin[1];
    		arrayMsg[0] = "";
    		for(String mg : arrayMsg) {
    			mensagem = mensagem +" "+ mg;
    		}
    		System.out.println(sendMsgToOne(remetente, destinatario, mensagem));
    		
    		return false;
    	}	
    	return true;
    }
    //metodo para enviar somente para um usuario
    private String sendMsgToOne(ClientSocket remetente, String destinatario, String msg) {
      
    	for(ClientSocket client : clientSocketList) {
    		
    		if(client.getLogin().equals(destinatario)) {
    			msg = remetente.getLogin() + " diz: " + msg;
    			client.sendMsg(msg);
    		}
    	}
    	return "Mensagem privada enviada para " + destinatario;
    }
